...
# 15.09.2026
# Most are exactly the same as in the repository
# Something we had had to change are some lines in createFeatureDF 
...
createFeatureDF <- function(modelList,
  ...
  for (i in base::seq_along(modelList)) {
    ...
    columnValues <- base::as.data.frame(modelList[[i]][["importance"]]) %>%
      ...
      #dplyr::arrange(dplyr::desc(dplyr::select(tidyselect::all_of(whichAccuracyMeasure)))) %>% # had to add dplyr select, otherwise it wouldnt work #didnt work stilll, asked chat and they said following row
...
calculateMissingGenes <- function(countDataRef,
  ...
  countDataNew[is.nan(countDataNew)] <- NA #added this, otherwise it failed
  countDataNew[is.infinite(countDataNew)] <- NA #added this, otherwise it failed
  ...
  for (i in unique(splits)) {
    combiSubset <- base::cbind(sesame::BetaValueToMValue(countDataRefSubset), countDataNewSubset[,splits == i, drop = F]) # changed this on april 1st, so maybe i have to rerun validation cohorts!!!!!
    #imputedDataComplete <- hush(impute::impute.knn(base::as.matrix(combiSubset), k = whichK)) #opens some weird connection, which leads to no results being printed to console anymore
