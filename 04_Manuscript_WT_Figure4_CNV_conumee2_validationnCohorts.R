# title: "CNV analysis retry"
# author: "Franziska Oberhammer"
# date: "2024-11-25"
# last worked on: "2025-11-10"
...
# details region
parameters$WT_associated_text_mining_path <- "/hpc/pmc_kuiper/Wilms/METHYLATION/DATA/BIN/C1333015_disease_gda_evidences.tsv"

source(file.path("/hpc/pmc_kuiper/Wilms/METHYLATION/CODE", "00_WT_colour_palettes2.R"))
...
loadAndPreprocessEPICsamplesToCNVdata <- function(dataFolder, sampleSheet, EPICVersion){
  ...
  if (EPICVersion == "EPICv1"){
    ...
    # some probes in EPIC array v2 have replicates. For these ones we just take the first CpG in the dataframe. In theory we should take the the probe with the lowest detection pValue but that was too difficult.
...
annotations$WT_associated_text_mining <- read.csv(parameters$WT_associated_text_mining, sep = "\t")

annotations$ensembl_dataset <- "hsapiens_gene_ensembl"

# Connect to the Ensembl Biomart
annotations$ensembl <- useMart("ensembl", dataset = annotations$ensembl_dataset)


annotations$WT_associated_genes <- getBM(attributes = c("external_gene_name" , "chromosome_name", "start_position", "end_position", "strand"),
                                         filters = "external_gene_name",
                                         values = unique(annotations$WT_associated_text_mining$Gene),
                                         mart = annotations$ensembl)

annotations$detail_regions <- GRanges(seqnames = paste0("chr", annotations$WT_associated_genes$chromosome_name),
                                      ranges = IRanges(start = annotations$WT_associated_genes$start_position,
                                                       end = annotations$WT_associated_genes$end_position),
                                      strand = annotations$WT_associated_genes$strand,
                                      name = annotations$WT_associated_genes$external_gene_name)

annotations$detail_regions_wo_chrXYH <- annotations$detail_regions[grepl(pattern = "chr\\d+", as.vector(annotations$detail_regions@seqnames))]
annotations$detail_regions_wo_chrXYH <- annotations$detail_regions_wo_chrXYH[!duplicated(annotations$detail_regions_wo_chrXYH$name)]

annotations$detail_regions_wo_chrXYH@seqnames <- droplevels(annotations$detail_regions_wo_chrXYH@seqnames)

annotations$detail_regions_wo_chrXYH@seqinfo



# create annotation objects
annotations$anno <- conumee2::CNV.create_anno(array_type = c("EPICv2"), 
                                              genome = "hg38")  

annotations$anno_with_details <- conumee2::CNV.create_anno(array_type = "EPICv2", 
                                                           exclude_regions = annotations$exclude_regions, 
                                                           detail_regions = annotations$detail_regions_wo_chrXYH, 
                                                           genome = "hg38",
                                                           chrXY = TRUE)
...
# patient_data$patient_V1_data <- qread(file.path(parameters$CNV_FOLDER, "CNVdata_EPICV1_InternalPatientIDs.qs")) (with old manifest (hg19))
patient_data$patient_V1_data <- qread(file.path(parameters$CNV_FOLDER, "CNVdata_EPICV1_InternalPatientIDs_hg38.qs")) #(with new manifest hg38)
...
## subset annotation for only probes that are on both arrays ----
annotations$probesOnBothArrays$anno <- annotations$anno
annotations$probesOnBothArrays$anno_with_details <- annotations$anno_with_details

annotations$probesOnBothArrays$anno@probes <- annotations$probesOnBothArrays$anno@probes[names(annotations$probesOnBothArrays$anno@probes) %in% probes_on_both_arrays]
annotations$probesOnBothArrays$anno_with_details@probes <- annotations$probesOnBothArrays$anno_with_details@probes[names(annotations$probesOnBothArrays$anno_with_details@probes) %in% probes_on_both_arrays]


## segmentation of the genome ---
# cnv_result_woDetail   <- segmentData(patient = patient_data$patient_combined, control = control_data$CNVdata, annotation = annotations$probesOnBothArrays$anno)
# cnv_result_withDetail <- segmentData(patient = patient_data$patient_combined, control = control_data$CNVdata, annotation = annotations$probesOnBothArrays$anno_with_details)
...
# parameters <- list()
parameters$PROJECT_FOLDER <- "/hpc/pmc_kuiper/Wilms/METHYLATION" # general folder where everything is stored
parameters$ANALYSIS_FOLDER <- file.path(parameters$PROJECT_FOLDER, "ANALYSIS") # relative path from project folder where you want your results to be
...
parameters$CURRENT_SAMPLESHEET          <- file.path(parameters$PROCESSED_DATA_FOLDER, 
                                                     "sample_sheet_updated_20250923.rds")
...
# Output folder
output_dir <- file.path(parameters$CNV_FOLDER, "plots", "Version_4.0", "summary")
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# Define clusters to plot
clusters <- c("1.a", "1.b", "2.a", "2.b", "2.c")

# Loop over clusters
for (cl in clusters) {
  filename <- file.path(output_dir, paste0("20250930_CNV_summary_", gsub("\\.", "", cl), ".pdf"))
  save_CNV_plot(cl, DNAm_5_cluster_letters, filename, sample_sheet, cnv_result_woDetail)
}

# Split 1.b manually into left/right
samples1bL <- c("CO", "BA", "CW", "CK", "AQ", "BH", "DI", "AV", "BG")
sample_sheet <- sample_sheet %>% 
  mutate(DNAm_5_cluster_letters_1bSplit = case_when(
    Internal_Patient_ID %in% samples1bL          ~ "1bL",
    DNAm_5_cluster_letters == "1.b"              ~ "1bR",
    TRUE                                         ~ DNAm_5_cluster_letters
  ))

# Plot split 1.b clusters
save_CNV_plot("1bL", DNAm_5_cluster_letters_1bSplit,
              file.path(output_dir, "20250930_CNV_summary_1bL.pdf"),
              sample_sheet, cnv_result_woDetail)

save_CNV_plot("1bR", DNAm_5_cluster_letters_1bSplit,
              file.path(output_dir, "20250930_CNV_summary_1bR.pdf"),
              sample_sheet, cnv_result_woDetail)
...
# CNV clustering Heatmap ---- 
sample_sheet_1b <- sample_sheet %>% 
  filter(DNAm_5_cluster_letters == "1.b")
cnv_result_woDetail_1b <- cnv_result_woDetail[sample_sheet_1b$Internal_Patient_ID]


# --- Prepare CNV bins and annotations ---
bins <- CNV.write(cnv_result_woDetail_1b, what = "bins")      # extract CNV bins
annotation <- bins[,c(1:4)]                                # keep chromosome/start/end info
annotation$X <- 1:nrow(annotation)                         # add index along genome
bins <- bins[,-c(1:4)]                                     # drop annotation columns
bins <- t(bins)                                            # transpose: rows = samples, cols = bins

# --- Chromosome boundaries and labels ---
b <- which(head(annotation$Chromosome, -1) != tail(annotation$Chromosome, -1))   # boundaries
l <- round(sapply(split(annotation$X, annotation$Chromosome), mean))             # midpoints
ll <- rep(NA, nrow(annotation))                                                  # placeholder
ll[l] <- names(l)                                                                # add chromosome names

# --- Color palette for CNV values ---
my_palette <- colorRampPalette(
  c("darkblue","darkblue", "white", "#F16729", "#F16729")
)(n = 1000)

# --- Sample clustering ---
bins.dist <- dist(bins, method = "manhattan")              # distance between samples
bins.hc <- hclust(bins.dist, method = "ward.D2")           # hierarchical clustering

# --- Consistency check ---
stopifnot(rownames(bins) == sample_sheet_1b$Internal_Patient_ID)

# --- Row annotations (sample clusters) ---
row_anno <- sample_sheet_1b$DNAm_5_cluster_letters
names(row_anno) <- row_anno
row_anno <- colours_WT$DNAm_clustering[row_anno]

# --- Heatmap ---
heatmap(bins, 
        Colv = NA, Rowv = as.dendrogram(bins.hc), scale = "n", useRaster = TRUE,
        main = "CNV Clustering",
        cexRow = 0.5, zlim = c(-0.5,0.5), col = my_palette,
        add.expr = abline(v=b), labCol = ll, cexCol = 1,
        RowSideColors = row_anno)
...
CNV_summaryTableList$CNV_summaryTable_1a <- generate_CNV_summarytable("1.a", sample_sheet, DNAm_5_cluster_letters, cnv_result_woDetail)
CNV_summaryTableList$CNV_summaryTable_1b <- generate_CNV_summarytable("1.b", sample_sheet, DNAm_5_cluster_letters, cnv_result_woDetail)
CNV_summaryTableList$CNV_summaryTable_2a <- generate_CNV_summarytable("2.a", sample_sheet, DNAm_5_cluster_letters, cnv_result_woDetail)
CNV_summaryTableList$CNV_summaryTable_2b <- generate_CNV_summarytable("2.b", sample_sheet, DNAm_5_cluster_letters, cnv_result_woDetail)
CNV_summaryTableList$CNV_summaryTable_2c <- generate_CNV_summarytable("2.c", sample_sheet, DNAm_5_cluster_letters, cnv_result_woDetail)
CNV_summaryTableList$CNV_summaryTable_1bL <- generate_CNV_summarytable("1bL", sample_sheet, DNAm_5_cluster_letters_1bSplit, cnv_result_woDetail)
CNV_summaryTableList$CNV_summaryTable_1bR <- generate_CNV_summarytable("1bR", sample_sheet, DNAm_5_cluster_letters_1bSplit, cnv_result_woDetail)
...
important_events_per_cluster$CNV_summaryTable_1bL
important_events_per_cluster$CNV_summaryTable_1bR


important_events_per_cluster_combined <- bind_rows(important_events_per_cluster, .id = 'id')


write.table(x = important_events_per_cluster_combined, file = file.path(parameters$CNV_FOLDER, "importantEventsPerCluster.csv"), row.names = F, quote = F, col.names = T)
...
# additional stuff ----
# CNV.detailplot(cnv_result_withDetail, name = "WT1")
# CNV.detailplot_wrap(cnv_result_withDetail)
